...
class Common(object):
    ...
    def get_expectation(self):
        return Fixed("E[%s]" % (self.variable_name, ), self.expectation, 1)


    # Obtains the variance, as a fixed variable
    def get_variance(self):
        return Fixed("Var[%s]" % (self.variable_name, ), self.variance, 1)
    ...
    def __eq__(self, another_variable):
        if overlaps(self, another_variable):
            return generate_true_fixed_var()
        else:
            return generate_false_fixed_var
...
class Fixed(Common):
    ...
    def __init__(self, given_variable_name, given_expectation, given_probability):
        Common.__init__(self, given_variable_name, "fixed", given_expectation, 0, given_expectation, given_expectation, given_probability)



# Generates a True variable, represented as a 1
def generate_true_fixed_var():
    return Fixed("TRUE", 1, 1)



# Generates a False variable, represented as a 1
def generate_false_fixed_var():
    return Fixed("FALSE", 0, 1)
